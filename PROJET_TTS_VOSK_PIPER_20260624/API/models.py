...
class Call(Base):
    ...
    ring_timeout_s = Column(Integer, nullable=True, default=60)
...
SCHEDULE_TYPES = {"once", "daily", "weekly", "monthly", "interval_30m", "interval_1h"}
...
def validate_ring_timeout_value(v: int) -> int:
    if v < 5 or v > 300:
        raise ValueError("Le timeout de sonnerie doit être entre 5 et 300 secondes")
    return v
...
def validate_tts_value(v: Optional[str]) -> Optional[str]:
    if v is None:
        return None
    v = v.strip()
    if not v:
        return None
    if len(v) > 1000:
        raise ValueError("Le texte TTS ne peut pas dÃ©passer 1000 caractÃ¨res")
    return v
...
class CallRequest(BaseModel):
    ...
    @validator("ring_timeout_s")
    def validate_ring_timeout(cls, v):
        return validate_ring_timeout_value(v)
