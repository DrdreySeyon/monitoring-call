...
const Utils = {
   ...
   */
  getStatusChip(status, label, reason) {
    const statusMap = {
      [CALL_STATUS.SUCCESS]: { class: "status-chip success", text: "Reussi" },
      [CALL_STATUS.FAILED]: { class: "status-chip failed", text: "Echec" },
      [CALL_STATUS.IN_PROGRESS]: { class: "status-chip in-progress", text: "En cours" },
      [CALL_STATUS.ANSWERED]: { class: "status-chip success", text: "Decroche / raccroche" },
      [CALL_STATUS.VOICEMAIL]: { class: "status-chip voicemail", text: "Messagerie vocale" },
      [CALL_STATUS.NO_ANSWER]: { class: "status-chip warning", text: "Non decroche" },
      [CALL_STATUS.BUSY]: { class: "status-chip warning", text: "Occupe" },
      [CALL_STATUS.REJECTED]: { class: "status-chip failed", text: "Rejete" },
      [CALL_STATUS.TRUNK_ERROR]: { class: "status-chip failed", text: "Erreur trunk" }
    };

    const info = statusMap[status] || { class: "status-chip", text: label || status || "-" };
    const title = reason || label || info.text;
    return `<span class="${info.class}" title="${this.escapeHtml(title)}">${this.escapeHtml(info.text)}</span>`;
  },
