...
const Tabs = {
  ...
  initElements() {
    ...
    this.tabSpeech = document.getElementById("speech");
  },
  ...
  onTabChanged(newTab, previousTab) {
    switch (newTab) {
      ...
      case "speech":
        if (window.loadSpeechValidation && typeof window.loadSpeechValidation === "function") {
          window.loadSpeechValidation();
        }
        break;
    }
   ...
   */
  nextTab() {
    const tabs = ["planner", "history", "stats", "scheduler", "speech"];
  ...
  loadTabState() {
    ...
    ["planner", "history", "stats", "scheduler", "speech"].forEach((tabName) => {
   ...
   */
  isValidTab(tabName) {
    return ["planner", "history", "stats", "scheduler", "speech"].includes(tabName);
  },
