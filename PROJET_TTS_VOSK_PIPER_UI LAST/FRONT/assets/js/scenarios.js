...
function bindScenarioImport() {
  const input = document.getElementById("scenarioImportInput");
  const templateButton = document.getElementById("downloadScenarioTemplateBtn");
  const exportButton = document.getElementById("exportScenariosBtn");

  templateButton?.addEventListener("click", downloadScenarioCsvTemplate);
  exportButton?.addEventListener("click", exportScenariosJson);
  input?.addEventListener("change", async () => {
    const file = input.files?.[0];
    if (!file) return;

    setScenarioImportMessage(`Import de "${file.name}" en cours...`, "warn");
    try {
      const result = await importScenariosFile(file);
      ...
      setScenarioImportMessage(
        `Import termine: ${created} cree(s), ${skipped} ignore(s)${errors.length ? `, ${errors.length} erreur(s)` : ""}.`,
        errors.length ? "warn" : "ok",
      );
    } catch (error) {
      setScenarioImportMessage(`Import non applique: ${error.message || "erreur technique"}.`, "warn");
    } finally {
      input.value = "";
    }
  });
...
function exportScenariosJson() {
  const url = `${CONFIG.apiBaseUrl}${CONFIG.apiPrefix}/scenarios/export`;
  const link = document.createElement("a");
  link.href = url;
  link.download = `scenarios_export_${new Date().toISOString().slice(0, 10)}.json`;
  link.click();
}

function setScenarioImportMessage(message, type) {
  const element = document.getElementById("scenarioImportMessage");
  if (!element) return;
  element.textContent = message;
  element.className = `form-message ${type === "ok" ? "is-ok" : "is-warn"}`;
}

function downloadScenarioCsvTemplate() {
  const headers = [
    "name",
    "keyword",
    "category",
  ...
  const rows = [
    ["Controle standard", "bonjour", "TEST", "+33123456789", "+33752049226", "TRUNK_SBC_SFR", "30", "60", "", "", "3000", "", "", "", "true"],
    ["Controle DTMF", "aide", "SVI", "+33123456789", "+33752049226", "TRUNK_SBC_SFR", "30", "60", "#12", "4", "3000", "", "daily", "2026-06-27T09:00", "true"],
  ...
  const link = document.createElement("a");
  link.href = url;
  link.download = "modele_import_scenarios.csv";
  link.click();
    ...
    .map(
      ...
      <article class="scenario-card">
        <div class="scenario-card-head">
          ...
          <div class="chip-row">${callModeChip(scenario)}<span class="chip ${Number(scenario.active) ? "chip-ok" : "chip-muted"}">${Number(scenario.active) ? "Actif" : "Inactif"}</span></div>
        </div>
