...
def serve_preview():
    preview_path = FRONT_ROOT / "preview.html"
    if not preview_path.exists():
        raise HTTPException(status_code=404, detail="Preview globale introuvable")
    return FileResponse(preview_path)


@app.get("/preview-modes-tts.html")
def serve_tts_modes_preview():
    preview_path = FRONT_ROOT / "preview-modes-tts.html"
    if not preview_path.exists():
        raise HTTPException(status_code=404, detail="Preview modes TTS introuvable")
    return FileResponse(preview_path)
...
def mos_label(score: Optional[float]) -> Optional[str]:
    if score is None:
        return None
    if score >= 4.3:
        return "Excellent"
...
def qos_status_from_mos(score: Optional[float]) -> Optional[str]:
    if score is None:
        return None
    if score >= 4.0:
        return "ok"
...
def cdr_packet_loss_pct(cdr_info: Optional[dict]) -> Optional[float]:
    loss_packets = safe_float(get_cdr_value(cdr_info, "rlp"))
    if loss_packets is None:
        return None

    rxcount = safe_float(get_cdr_value(cdr_info, "rxcount"))
    if rxcount is not None and rxcount > 0:
        return round(clamp((loss_packets / (rxcount + loss_packets)) * 100, 0.0, 100.0), 2)

    if 0 <= loss_packets <= 1:
        return round(loss_packets * 100, 2)

    if 1 < loss_packets <= 100:
        return round(loss_packets, 2)

    return None


def estimate_mos(cdr_info: Optional[dict]) -> Optional[float]:
    jitter = mean_available(get_cdr_value(cdr_info, "rxjitter"), get_cdr_value(cdr_info, "txjitter"))
    loss = cdr_packet_loss_pct(cdr_info)
    rtt = safe_float(get_cdr_value(cdr_info, "rtt"))

    if jitter is None and loss is None and rtt is None:
        return None

    latency_penalty = max(0.0, ((rtt or 0.0) - 150.0) / 10.0)
    jitter_penalty = max(0.0, ((jitter or 0.0) - 20.0) / 5.0)
    loss_penalty = max(0.0, (loss or 0.0) * 2.5)
    r_factor = clamp(93.2 - latency_penalty - jitter_penalty - loss_penalty, 0.0, 100.0)
    mos = 1 + 0.035 * r_factor + 0.000007 * r_factor * (r_factor - 60) * (100 - r_factor)
    return round(clamp(mos, 1.0, 4.5), 2)


def serialize_quality(cdr_info: Optional[dict]) -> dict:
    jitter = mean_available(get_cdr_value(cdr_info, "rxjitter"), get_cdr_value(cdr_info, "txjitter"))
    loss = cdr_packet_loss_pct(cdr_info)
    rtt = safe_float(get_cdr_value(cdr_info, "rtt"))
    score = estimate_mos(cdr_info)
    return {
        "rtp_jitter_ms": round(jitter, 2) if jitter is not None else None,
        "rtp_packet_loss_pct": loss,
        "rtp_rtt_ms": round(rtt, 2) if rtt is not None else None,
        "rtp_rxjitter_ms": safe_float(get_cdr_value(cdr_info, "rxjitter")),
        "rtp_txjitter_ms": safe_float(get_cdr_value(cdr_info, "txjitter")),
        "rtp_rxcount": safe_int(get_cdr_value(cdr_info, "rxcount")),
        "rtp_txcount": safe_int(get_cdr_value(cdr_info, "txcount")),
        "rtp_lost_packets": safe_int(get_cdr_value(cdr_info, "rlp")),
        "rtp_rxmes": safe_float(get_cdr_value(cdr_info, "rxmes")),
        "rtp_txmes": safe_float(get_cdr_value(cdr_info, "txmes")),
        "mos_score": score,
        "mos_label": mos_label(score),
        "qos_status": qos_status_from_mos(score),
    }
...
def fetch_cdr_details_map(channel_ids: list[str], db: Session) -> dict:
    ...
    try:
        statement = text(
            ...
            "rxjitter, txjitter, rlp, rtt, rxcount, txcount, rxmes, txmes "
            "FROM cdr "
    ...
    for row in rows:
        ...
        if uniqueid and uniqueid not in details:
            details[uniqueid] = {
                ...
                "rxjitter": row.get("rxjitter"),
                "txjitter": row.get("txjitter"),
                "rlp": row.get("rlp"),
                "rtt": row.get("rtt"),
...
def parse_import_bool(value, default=False) -> bool:
    ...
    if isinstance(value, (int, float)):
        return bool(value)
    return str(value).strip().lower() in {"1", "true", "vrai", "yes", "oui", "y", "on", "actif"}


def parse_import_int(value, default=None):
    if value is None or value == "":
        return default
...
def parse_scenario_import_file(filename: str, raw: bytes) -> list[dict]:
    ...
    if filename.lower().endswith(".csv"):
        ...
        if not reader.fieldnames:
            raise HTTPException(status_code=400, detail="CSV invalide: entetes manquantes")
        return [
...
def normalize_scenario_import_item(item: dict) -> ScenarioCreate:
    normalized = {field: clean_import_text(item.get(field)) for field in SCENARIO_IMPORT_FIELDS}
    normalized["call_time_s"] = parse_import_int(normalized.get("call_time_s"), 30)
    normalized["ring_timeout_s"] = parse_import_int(normalized.get("ring_timeout_s"), 60)
    normalized["time_s_before_dtmf"] = parse_import_int(normalized.get("time_s_before_dtmf"))
    normalized["time_ms_between_dtmf"] = parse_import_int(normalized.get("time_ms_between_dtmf"), 3000)
    normalized["schedule_enabled"] = parse_import_bool(normalized.get("schedule_enabled"), False)
    normalized["prevent_overlap"] = parse_import_bool(normalized.get("prevent_overlap"), True)
    normalized["retry_enabled"] = parse_import_bool(normalized.get("retry_enabled"), False)
    normalized["active"] = parse_import_bool(normalized.get("active"), True)

    if normalized.get("frequency") and normalized.get("schedule_date") and not normalized["schedule_enabled"]:
        normalized["schedule_enabled"] = True
    if normalized.get("frequency") and not normalized.get("schedule_type"):
        normalized["schedule_type"] = normalized["frequency"]

    return ScenarioCreate(**normalized)
...
async def import_scenarios(
    ...
    try:
        raw = await file.read()
        scenarios_data = parse_scenario_import_file(file.filename or "", raw)
        ...
        for idx, item in enumerate(scenarios_data, start=1):
            try:
                ...
                scenario = normalize_scenario_import_item(item)

                existing = db.query(Scenario).filter(
                    Scenario.name == scenario.name,
                    Scenario.caller == scenario.caller,
                    Scenario.callee == scenario.callee,
                    Scenario.trunk == scenario.trunk
                ).first()
        ...
        return {
            ...
            "skipped": skipped,
            "errors": errors,
            "items": [serialize_scenario(s) for s in imported],
        }
