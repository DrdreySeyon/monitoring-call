...
function schedulerScenarioCell(job) {
  ...
  const tts = scenario?.tts || job.tts || "";
  return `<strong>${escapeHtml(name)}</strong><br><small class="muted">${escapeHtml(category)} - ${escapeHtml(job.id || "-")}</small>${tts ? `<br><small class="muted">TTS: ${escapeHtml(tts)}</small>` : ""}`;
}

function schedulerRunStatusChip(status) {
  const normalized = String(status || "").toLowerCase();
  const map = {
    success: ["chip-ok", "Reussi"],
    running: ["chip-info", "En cours"],
    failed: ["chip-ko", "Erreur"],
    skipped: ["chip-warn", "Ignore"],
    schedule_error: ["chip-ko", "Erreur planif"],
  };
  const [cls, label] = map[normalized] || ["chip-muted", status || "Jamais execute"];
