function renderHealth() {
  ...
  const ttsSummary = getTtsHealthSummary();
  ...
  const items = [
    ...
    ["TTS / Piper", `${ttsSummary.scenarios} scenarios`, ttsSummary.scenarios ? "pill-info" : "pill-muted"],
  ];
  ...
  const details = [
    ...
    ["TTS / Piper", `Pilote par Asterisk | Scenarios TTS: ${ttsSummary.scenarios} | Appels TTS charges: ${ttsSummary.calls} | TTS OK: ${ttsSummary.ok}`],
  ];
...
function getTtsHealthSummary() {
  const scenarios = state.scenarios || [];
  const calls = state.calls || [];
  return {
    scenarios: scenarios.filter((scenario) => String(scenario.tts || "").trim()).length,
    calls: calls.filter((call) => ["tts", "tts_dtmf"].includes(callMode(call))).length,
    ok: calls.filter((call) => ttsStatus(call) === "ok").length,
  };
}
