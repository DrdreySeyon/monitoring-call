...
function bindHistoryFilters() {
  ...
  document.getElementById("historyModeFilter").addEventListener("change", renderFullHistory);
...
function renderTable() {
  ...
  state.filteredCalls.forEach((call) => {
    ...
    row.innerHTML = `
      ...
      <td><div class="chip-row">${mosChip(call)}</div><small class="muted">${escapeHtml(qualitySummary(call))}</small></td>
...
function getFilteredHistoryCalls() {
  ...
  return state.calls.filter((call) => {
    ...
    return (!query || haystack.includes(query)) && categoryOk && callStatusOk && voskOk && modeOk && dateOk;
  });
}
...
function exportFilteredHistoryCsv() {
  ...
  const rows = calls.map((call) => [
    ...
    callModeLabel(callMode(call)),
    call.vosk_status || "",
    call.mos_score ?? "",
    call.mos_label || "",
    call.qos_status || "",
    call.rtp_jitter_ms ?? "",
    call.rtp_packet_loss_pct ?? "",
    call.rtp_rtt_ms ?? "",
    call.rtp_rxjitter_ms ?? "",
    call.rtp_txjitter_ms ?? "",
    call.rtp_rxcount ?? "",
    call.rtp_txcount ?? "",
    call.rtp_lost_packets ?? "",
    ttsStatus(call),
    call.tts || "",
    call.dtmf || "",
    call.duration ?? "",
...
function renderDetail() {
  ...
  detail.innerHTML = `
    <div class="detail-section">
      <h3>Statut appel</h3>
      <div class="chip-row">${statusChip(call)}${callModeChip(call)}${call.sip_error_code ? `<span class="chip chip-ko">SIP ${escapeHtml(call.sip_error_code)}</span>` : ""}</div>
    ...
    <div class="detail-section">
      ...
      <div class="chip-row">${voskChip(call)}${ttsChip(call)}</div>
    ...
    <div class="detail-section">
      <h3>Qualite voix</h3>
      <div class="chip-row">${mosChip(call)}</div>
      ${kv("Jitter moyen", formatMs(call.rtp_jitter_ms))}
      ${kv("Jitter RX", formatMs(call.rtp_rxjitter_ms))}
      ${kv("Jitter TX", formatMs(call.rtp_txjitter_ms))}
      ${kv("Perte paquets", formatPercent(call.rtp_packet_loss_pct))}
      ${kv("Paquets perdus", call.rtp_lost_packets ?? "-")}
      ${kv("RTT", formatMs(call.rtp_rtt_ms))}
      ${kv("Paquets RX", call.rtp_rxcount ?? "-")}
      ${kv("Paquets TX", call.rtp_txcount ?? "-")}
      ${kv("QoS", call.qos_status || "-")}
    </div>
    <div class="detail-section">
      <h3>Scenario</h3>
...
function renderTimeline() {
  ...
  const steps = [
    ...
    ["Mode", callModeLabel(callMode(call))],
    ["ARI", `Appel cree vers ${call.callee || "-"}`],
    ["TTS", call.tts ? `${call.tts} - ${ttsStatus(call)}` : "Aucun TTS"],
    ...
    ["Qualite voix", `${call.mos_score ? `MOS ${formatMos(call.mos_score)} - ${call.mos_label || "non qualifie"}` : "MOS non renseigne"} - ${qualitySummary(call)}`],
