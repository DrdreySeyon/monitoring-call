const Scheduler = {
  ...
  init() {
    this.initElements();
    this.bindEvents();
    console.log("Module Scheduler initialise");
  },
    ...
    try {
      ...
      if (this.tableBody) {
        this.tableBody.innerHTML = `
          <tr>
            <td colspan="10">Erreur de chargement du scheduler</td>
          </tr>
        `;
      }
      Utils.showToast(error.message || "Erreur scheduler", "error");
    } finally {
    ...
    this.tableBody.innerHTML = this.items.map((item) => {
      ...
      return `
        <tr>
          ...
          <td>${Utils.escapeHtml(schedule.schedule_date || "-")}</td>
  ...
  async sync() {
    try {
      ...
      Utils.showToast("Scheduler synchronisé", "success");
      await this.load();
    } catch (error) {
