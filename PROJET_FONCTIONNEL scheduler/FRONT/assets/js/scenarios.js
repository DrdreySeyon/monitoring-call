const Scenarios = {
  ...
  render() {
    ...
    this.tableBody.innerHTML = this.scenarios.map((s) => {
      ...
      <tr>
        ...
        <td>${Utils.escapeHtml(s.schedule?.schedule_time || "-")}</td>
  ...
  buildPayload() {
    ...
    if (scheduleEnabled && frequency === "once" && !scheduleDate) {
      throw new Error("La date unique est obligatoire pour une planification une fois");
    }
