...
def parse_start_date(schedule_date: Optional[str]) -> Optional[datetime]:
    if not schedule_date:
        return None
    return datetime.fromisoformat(schedule_date)


def weekday_from_date(value: datetime) -> str:
...
def build_trigger(scenario: Scenario):
    ...
    start_date = parse_start_date(scenario.schedule_date)
    hour, minute = (
        (start_date.hour, start_date.minute)
        if start_date
        else parse_time(scenario.schedule_time)
    )
    ...
    if schedule_type == "once":
        ...
        return DateTrigger(run_date=start_date, timezone=timezone)

    if schedule_type == "daily":
        return CronTrigger(hour=hour, minute=minute, start_date=start_date, timezone=timezone)

    if schedule_type == "interval_30m":
        return IntervalTrigger(minutes=30, start_date=start_date, timezone=timezone)

    if schedule_type == "interval_1h":
        return IntervalTrigger(hours=1, start_date=start_date, timezone=timezone)

    if schedule_type == "weekly":
        day = weekday_from_date(start_date) if start_date else (scenario.schedule_day_of_week or "mon").strip().lower()
        return CronTrigger(day_of_week=DAY_MAP.get(day, day), hour=hour, minute=minute, start_date=start_date, timezone=timezone)

    if schedule_type == "monthly":
        day = start_date.day if start_date else 1
        return CronTrigger(day=day, hour=hour, minute=minute, start_date=start_date, timezone=timezone)
...
def run_scenario_job(scenario_id: int, manual: bool = False) -> dict:
    ...
    try:
        ...
        call_row, ari_response = perform_call(
            ...
            ring_timeout_s=scenario.ring_timeout_s or 60,
            dtmf=scenario.dtmf,
